            ...
            if (body.message.reply_to_message) {
                if (getChatMember(body.message.chat.id, Const.botId).can_restrict_members) {
                    ...
                    if (text.indexOf("s") >= 0) {
                        ...
                        sendMessage.text = "Deleted Message " + body.message.reply_to_message.message_id + "\n\n\nSelf-destruct in a few seconds later. "
                        postTelegram(delMsg)
                        var deleteSentMsg = postTelegram(sendMessage)
                        if(deleteSentMsg.ok == true){
                            var deleteSentPayload = {
                                "method": "deleteMessage", 
                                "chat_id": deleteSentPayload.result.chat.id,
                                "message_id": deleteSentPayload.result.message_id
                            }
