function handlePin(body) {
    ...
    if (isAdminOrCreator(body.message.from.id, body.message.chat.id)) {
        if (!param[1]) {
            if (body.message.reply_to_message  && text.indexOf("all") <=0){
                if (getChatMember(body.message.chat.id, Const.botId).can_pin_messages) {
                    var pinMsg= {
                    ...
                    if (text.indexOf("s") >= 0) {
                        ...
                        var chatid = body.message.chat.id.toString()
                        sendMessage.text = "I have pinned [this message](" + Const.tgLink + "c/" + chatid.replace("-100","") + "/" + body.message.reply_to_message.message_id + ")\n\nSelf-destruct in a few seconds later. "
                        postTelegram(pinMsg)
                        var deleteSentMsg = postTelegram(sendMessage)
                        if(deleteSentMsg.ok == true){
                            var deleteSentPayload = {
